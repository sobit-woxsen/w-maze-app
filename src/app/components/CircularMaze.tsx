...
export default function AdvancedCircularMaze() {
  const [maze, setMaze] = useState<Cell[][]>([]);
  const [playerPos, setPlayerPos] = useState<Point>({ ring: 0, sector: 0 });
  const [gameWon, setGameWon] = useState(false);
  ...
  const movePlayer = useCallback(
    (direction: "in" | "out" | "clockwise" | "counterClockwise") => {
      ...
      setPlayerPos((prevPos) => {
        ...
        if (newPos.ring === 0) {
          setGameWon(true);
          setBestScore((prevBest) => Math.min(prevBest, moves + 1));
        }
      ...
      <div>
        <div className="flex flex-col items-center space-y-4">
          ...
          {/* <div className="text-sm text-gray-600">
            <p>Moves: {moves}</p>
            <p>Best Score: {bestScore === Infinity ? "N/A" : bestScore}</p>
          </div>
          <p className="text-sm text-gray-600">
            Use arrow keys or WASD to move. Reach the red circle in the center
            to win!
